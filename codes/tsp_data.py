"""TSP Dataset:eil51, berlin52, st70, kroA100"""
...
# 
_INSTANCE_REGISTRY: dict[str, list[tuple[int, int]]] = {
    'eil51':   EIL51_COORDS,
...
def compute_distance_matrix(coords: list[tuple[int, int]]) -> np.ndarray:
    coords_arr = np.array(coords, dtype=np.float64)  # shape (n, 2)
    # Broadcasting: diff[i,j] = coords[i] - coords[j]
    diff = coords_arr[:, np.newaxis, :] - coords_arr[np.newaxis, :, :]  # (n, n, 2)
    distances = np.sqrt((diff ** 2).sum(axis=2))
    dist = np.floor(distances + 0.5).astype(np.int32)                   # (n, n)
    return dist
...
def nearest_neighbor_tour(dist_matrix: np.ndarray, start: int = 0) -> list[int]:
    ...
    for _ in range(n - 1):
        current = tour[-1]
        row = dist_matrix[current].astype(np.float64)  #fix OverflowError
        row[visited] = np.inf
...
def get_instance(name: str) -> tuple[list, np.ndarray, int]:
    if name not in _INSTANCE_REGISTRY:
        available = sorted(_INSTANCE_REGISTRY.keys())
        raise ValueError(
            f"Unknown TSP instance '{name}'. "
            f"Available instances: {available}"
        )
    coords = _INSTANCE_REGISTRY[name]
